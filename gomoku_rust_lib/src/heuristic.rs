...
pub fn heuristic(state: &mut State) -> i32 {
    ...
    value += assign_flank_value_to_state(state, &board_state_info);
...
fn assign_flank_value_to_state(state: &mut State, board_state_info: &BoardStateInfo) -> i32 {
	let mut value: i32 = 0;
...
fn assign_capture_value_to_state(state: &mut State, board_state_info: &BoardStateInfo) -> i32 {
	...
	if state.current_player == 1 {state.white_captured_stone += board_state_info.stone_captured;}
	else {state.black_captured_stone += board_state_info.stone_captured;}
	if state.current_player == 1 {eat_count = state.white_captured_stone;}
	else {eat_count = state.black_captured_stone;}
	if eat_count >= 10 {
		value += global_var::HEURISTIC_MAX_VALUE;
	}
	else {
