...
static PATTERN: [(u8,usize,&str); 12] = [(0xF8, 5, "five"), // five XXXXX...
...
// static closesplittriple: u8 = 0xD0; // XX.X.

pub fn patern_finder(bitboards: &Bitboards, pos: usize) {
	println!("pos: {}", pos);
	println!("x idx: {}", pos/19);
	println!("y idx: {}", pos%19);
	println!("int idx: {}", pos/64);
	let y = pos % 19;
	let mut int_to_check = create_row(bitboards,pos);
...
fn create_row(bitboards: &Bitboards, pos: usize) -> u64 {
	let mask: [u64; 24] = [
	...
	if (row_idx ==3||row_idx ==6||row_idx ==10||row_idx ==13||row_idx ==16) {
		row = (((bitboards.white_board[int_idx] & mask[row_idx]) << shift) | ((bitboards.white_board[int_idx + 1] & mask[19 + int_idx]) >> (64 - shift)));
	}
	else {
		row = (bitboards.white_board[int_idx] & mask[row_idx]) << shift;
	}
	return row >> 45;
}