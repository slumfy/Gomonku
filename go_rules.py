...
class Go:
	...
	def check_eat_in_table(self,player,x,y,xsign,ysign,offset):
		if x + offset * xsign > 18 or x + offset * xsign < 0 or y + offset * ysign > 18 or y + offset * ysign < 0:
			return 1
		return 0

	def eat_position_routine(self,player,x,y,xsign, ysign):
		if self.check_eat_in_table(player,x,y,xsign,ysign,1) == 0 and self.table[x + 1 * xsign][y + 1 * ysign] != player.nb and self.table[x + 1 * xsign][y + 1 * ysign] != 0:
			if self.check_eat_in_table(player,x,y,xsign,ysign,2) == 0 and self.table[x + 2 * xsign][y + 2 * ysign] == player.nb:
				self.eat_stone(player,[(x + 1 * xsign, y + 1 * ysign)])
			elif self.check_eat_in_table(player,x,y,xsign,ysign,2) == 0 and self.table[x + 2 * xsign][y + 2 * ysign] != 0:
				if self.check_eat_in_table(player,x,y,xsign,ysign,3) == 0 and self.table[x + 3 * xsign][y + 3 * ysign] == player.nb:
					self.eat_stone(player, [(x + 1 * xsign, y + 1 * ysign), (x + 2 * xsign, y + 2 * ysign)])
